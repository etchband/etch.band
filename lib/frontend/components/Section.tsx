'use client'
import Box from '@cluesurf/leaf/component/Box'
import {
  A,
  Em,
  H1,
  ...
  P,
  Strong,
} from '@cluesurf/leaf/component/Content'
...
export default function Section({
  ...
  useScripts(['code', 'emoji'].concat(scripts))

  return (
    <SectionContext.Provider value={{ layout: cached.layout, scope }}>
      <Layout>
...
function Content({ children }: { children: React.ReactNode }) {
  ...
  return (
    <div
      className={clsx(
        layout === 'document' && 'pt-64',
        'flex-1 min-h-full flex flex-col',
        'print:pt-0 print:pb-0',
      )}
    >
      {children}
    </div>
  ...
  return (
    <header
      className={clsx(
        'flex-1 section',
        'print:break-after',
        'print:min-h-screen print:justify-center',
        ...
        'print:pb-0 print:pt-64 print:border-none',
      )}
    >
      {children}
    </header>
  ...
  return (
    <section
      className={clsx(
        ...
        'print:pb-0 print:pt-0 print:border-none',
        'border-0 border-b-8 border-zinc-50 dark:border-zinc-900 border-solid last:border-none',
      )}
    >
      {children}
...
Section.H1 = ({ children }: { children: React.ReactNode }) => {
  return <H1 align="center">{children}</H1>
}
...
Section.List = ({
  ...
  return (
    <Tag className="rounded-sm overflow-hidden mb-32 mx-16 !mt-16 list-none [&>li:nth-child(odd)]:bg-zinc-100 [&>li:nth-child(even)]:bg-zinc-50 dark:[&>li:nth-child(odd)]:bg-zinc-800 dark:[&>li:nth-child(even)]:bg-zinc-900">
      {children}
    </Tag>
  )
}
...
Section.Item = ({
  ...
  const { scope } = useContext(SectionContext)
  const label = React.isValidElement(providedLabel)
    ? providedLabel
    : providedLabel
    ? renderComponent(providedLabel, scope)
    : undefined

  return (
    <li
      className={clsx(
        'flex flex-wrap justify-center gap-16 py-16 px-16 print:px-0 dark:text-zinc-400 dark:font-medium',
        className,
      )}
    >
...
function Title({
  ...
  assert(
    HEADING_LEVEL.includes(level),
    `Heading level ${level} is not supported`,
  )

  switch (level) {
    case 2:
      return <Section.H2>{children}</Section.H2>
    case 3:
      return <Section.H3>{children}</Section.H3>
    case 4:
    ...
    case 1:
    default:
      return <Section.H1>{children}</Section.H1>
  }
}
